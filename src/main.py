...
def main():
    ...
    async def parse_sender(
        ...
        text = (
            "**----------[BURGER TOWN ORE TRACKER]----------**"
            + f"\n**{username}** "
            + f"has found **{(ore_type.capitalize() + ' ' if ore_type != 'NORMAL' else '')}{' '.join(ore_name) if isinstance(ore_name, list) else ore_name}** "
            + f"{('' if cave_type is None else '(' + str(cave_type) + ') ')}[**{tier}**]"
            + f"\nWorld: {world}"
            + f"\nBase Rarity: 1/{base_rarity:,} {'in ' + cave_type + 's' if cave_type != None else ''}"
            + f"\nBlocks Mined: {blocks_mined:,}"
            + f"\nPickaxe: {pickaxe}"
            + f"\nEvent: {event}"
            + (
                f"\nAdjusted: {int(CAVE_ORES[str(cave_type)]['rarity'] * 1.88 * (CAVE_ORES[str(cave_type)]['ores'][ore_name][TYPE_INDEX[ore_type]]) if cave_type != 'Gilded Cave' else base_rarity * 1.88 * ((5700) if pickaxe != '57 Leaf Clover' else (57)) * ((2.5) if ore_name != 'Hyperheated Quasar' else (1))):,}"
    ...
    async def _bot_scmd_manual_track(
        ...
        try:
            ...
            if str(ctx.author.id) == "598815178141990913":
